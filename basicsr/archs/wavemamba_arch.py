...
class ffn(nn.Module):
    ...
    def forward(self, x):
        x = self.conv2(self.conv1(x))  # 深度可分离卷积
        x1, x2 = x.chunk(2, dim=1)
...
class UNet(nn.Module):
    def __init__(self, in_chn=3, wf=16, n_l_blocks=[1, 2, 2, 4], ffn_scale=2, up_mode='upconv', conv1=conv):  # wf=48
    ...
    def forward(self, x):
        ...
        # 低频恢复
        blocks = []
        x0 = self.layer0(x_LL)  # 浅层特征提取，不改变尺寸 16
        x0 = self.down_group0(x0)  # Mamba
        blocks.append(x0)

        x1 = self.layer1(x0)  # 开始下采样 32
        x1 = self.down_group1(x1)  # 经过Mamba块
        blocks.append(x1)

        x2 = self.layer2(x1)  # 开始下采样 64
        x2 = self.down_group2(x2)  # 经过Mamba块
        blocks.append(x2)

        x3 = self.layer3(x2)  # 开始下采样 128
        x3 = self.down_group3(x3)  # 经过Mamba块
        blocks.append(x3)

        x4 = self.layer4(x3)
        x4 = self.down_group4(x4)  # 256
...
# #
# #
# # class SS2D(nn.Module):  # Vision State Space Module
# #     def __init__(
# #             self,
# #             d_model,
# #             d_state=16,
...
# #
# #
# # class VSSBlock(nn.Module):  # Residual State Space Block
# #     def __init__(
# #             self,
# #             hidden_dim: int = 0,
# #             drop_path: float = 0,
...
# #
# #         # 第一阶段，只处理低频成分
# #         x_LL_first = self.conv_first(x_LL)
# #         res = self.conv_after_body(self.forward_features(x_LL_first)) + x_LL_first
# #         x_LL = x_LL + self.conv_last(res)
# #
# #         x_stage1 = idwt(torch.cat((x_LL, x_high0), dim=0))
# #
# #         # 第二阶段，对高频细节进行增强
# #         x_final = self.DE(x_stage1)
...
# #     def __init__(self):
# #         super().__init__()
# #         self.restoration_network = MambaIR()
# #
# #     def print_network(self, model):
# #         num_params = 0
# #         for p in model.parameters():
